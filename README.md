...
적 AI는 `Behavior Tree`와 `Blackboard`를 기반으로 구현했습니다.
...
AI는 플레이어와의 거리, 공격 가능 여부, 현재 상태에 따라 행동을 전환합니다.

이동은 기본적으로 NavMesh 기반 `MoveTo`를 사용하며,  
이동 실패 상황에서는 `AddMovement`를 활용해 보정했습니다.
